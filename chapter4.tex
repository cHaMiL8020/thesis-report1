...
This thesis implements an end-to-end neuro-symbolic anomaly detection pipeline for hourly Austrian electricity-market and grid data under weather uncertainty. At a high level, the framework transforms heterogeneous raw observations into increasingly structured representations \cite{abeysekara2025report,repo_readme_framework}:
\FloatBarrier
\begin{figure}[H]
	\centering
...
Once the multivariate forecast $\hat{\mathbf{y}}_t$ is available, the framework computes residuals as the primary statistical signal for anomaly generation \cite{abeysekara2025report}:
\begin{equation}
...
After feature engineering, the first trainable element in the neural pipeline is the dCeNN encoder. It can characterizes this module as a lightweight Tiny dCeNN temporal encoder, while the repository implements it as a compact masked recurrent encoder trained through an autoencoding objective \cite{abeysekara2025report,repo_train_dcenn_elm}.
...
Methodologically, this veto mechanism addresses a specific weakness of purely statistical anomaly detection: the tendency to over-flag surprising points that are still physically plausible, contextually explainable, or economically trivial. This issue is identified as one of the main gaps in prior practice. The ASP stage is the thesis response to that gap. It reduces false positives not by tuning thresholds further, but by subjecting the flagged cases to domain-aware reasoning \cite{abeysekara2025report}.
