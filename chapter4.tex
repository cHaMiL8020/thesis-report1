...
This thesis implements an end-to-end neuro-symbolic anomaly detection pipeline for hourly Austrian electricity-market and grid data under weather uncertainty. The complete implementation of the proposed framework is publicly available at \url{https://github.com/cHaMiL8020/thesis-grid-anomaly}. At a high level, the framework transforms heterogeneous raw observations into increasingly structured representations \cite{repo_thesis_grid_anomaly}:
\FloatBarrier
\begin{figure}[H]
	\centering
...
In implementation terms, the ASP script reads the anomaly table and the engineered feature table, converts selected quantities into symbolic facts, and invokes \texttt{clingo} over the rule base in \texttt{rules/market\_rules.lp} \cite{repo_thesis_grid_anomaly}. The generated facts include anomaly indicators, holiday information, shortwave radiation, wind power, actual load, and a reference load baseline. The solver then returns two explicit categories:
\begin{itemize}
...
After feature engineering, the first trainable element in the neural pipeline is the dCeNN encoder. It can characterizes this module as a lightweight Tiny dCeNN temporal encoder, while the repository implements it as a compact masked recurrent encoder trained through an autoencoding objective \cite{repo_thesis_grid_anomaly}.
...
\end{equation}
where $\gamma$ is a residual mixing coefficient. In the actual training script, this coefficient is implemented as $0.3$, which provides stability-oriented residual mixing in each recurrent update \cite{repo_thesis_grid_anomaly}.
...
\end{itemize}
This means that \texttt{elm\_heads.npz} is the complete deployable forecasting head. During inference, the model only needs the encoder embedding $\mathbf{H}_t$ and this stored weight matrix to generate multivariate predictions \cite{repo_thesis_grid_anomaly}.
...
In the repository, this logic is implemented in \texttt{src/07\_apply\_asp.py}, which reads the anomaly table produced by the detection stage, generates symbolic facts from both the anomaly outputs and the engineered data, invokes \texttt{clingo}, and writes a refined anomaly table containing confirmed and vetoed cases \cite{repo_thesis_grid_anomaly}. The associated rule base is implemented in \texttt{rules/market\_rules.lp} \cite{repo_thesis_grid_anomaly}. Although the user prompt refers to \texttt{07\_asp\_rules.lp}, the current repository operationalizes the same symbolic role through \texttt{market\_rules.lp}; this file is the actual ASP knowledge base used by the pipeline.
...
The current rule base in \texttt{rules/market\_rules.lp} is intentionally compact but methodologically rich \cite{repo_thesis_grid_anomaly}. It defines five main rule families.
...
\end{align}
Here, the constant \texttt{wind\_ramp\_limit} is set to 500 in the rule file \cite{repo_thesis_grid_anomaly}. This gives the rule base a concrete sanity bound.
...
\end{itemize}
The final refined anomaly table then stores, for each retained or rejected item, the target, timestamp, hour index, anomaly score, final flag, and veto indicator \cite{repo_thesis_grid_anomaly}.
...
This logic is also evident in the repository’s small debugging script, which manually injects a ``solar anomaly at night'' fact together with zero radiation and tests the rule base through \texttt{clingo} \cite{repo_thesis_grid_anomaly}. Although minimal, this script illustrates the exact spirit of the symbolic component: anomaly claims should be testable against explicit logic rather than accepted as opaque outputs.
...
\begin{enumerate}
	\item \texttt{src/05\_detect\_anomalies.py}, which reconstructs the supervised test set, loads the trained neural artifacts, computes predictions and residuals, applies calibrated thresholds, and writes a full anomaly table \cite{repo_thesis_grid_anomaly};
	\item \texttt{src/07\_apply\_asp.py}, which reads that anomaly table together with the engineered feature table, converts selected values into facts, runs \texttt{clingo}, and writes the refined anomaly table \cite{repo_thesis_grid_anomaly}.
\end{enumerate}
...
The first half of the hybrid integration is the construction of the neural anomaly table. In \texttt{src/05\_detect\_anomalies.py}, the trained encoder, ridge head, scaler, and threshold artifacts are reloaded, and the supervised test matrix is rebuilt from the engineered CSV using the same lag and rolling configuration used earlier in the pipeline \cite{repo_thesis_grid_anomaly}. The dCeNN encoder produces latent representations, the ELM head generates multivariate forecasts, and the detector computes residuals:
\begin{equation}
	\mathbf{r}_t = \mathbf{y}_t - \hat{\mathbf{y}}_t.
...
The helper function \texttt{\_generate\_facts(...)} in \texttt{src/07\_apply\_asp.py} creates a string program of ASP facts from the anomaly table and engineered data \cite{repo_thesis_grid_anomaly}. Four main fact families are produced.
