...
\begin{enumerate}
	...
	\item \textbf{deployment feasibility}, meaning that the trained model can be exported into a compact and reproducible artifact suitable for transfer to hardware such as a Raspberry Pi, Jetson-class device, or lightweight gateway \cite{repo_thesis_grid_anomaly}.
\end{enumerate}
...
YAML-based configuration is especially suitable here because it separates \emph{experimental policy} from \emph{execution logic}. A configuration-driven strategy of this kind is particularly valuable in modular machine-learning systems, where explicit parameter management supports both reproducibility and scalable experimentation \cite{bousbiat:TII2022}. The source code defines how the pipeline works; the YAML files define under what settings it is run. This separation reduces code duplication, makes reruns easier to document, and improves experimental traceability. It also aligns well with the modular software architecture discussed in Section~6.1.
...
This file defines global data paths, the engineered dataset path, the holiday CSV path, the target NPZ artifact path, the chronological split boundaries, and the forecast horizon \cite{repo_thesis_grid_anomaly}. In effect, it controls the global experimental frame within which all other hyperparameters are interpreted.

\paragraph{Feature configuration (\texttt{features.yaml}).}
This file controls the supervised-feature construction regime: lag offsets, rolling windows, and winsorized columns \cite{repo_thesis_grid_anomaly}. These settings determine the temporal receptive field of the forecasting model and the robustness of the feature representation against extreme raw values.

\paragraph{dCeNN configuration (\texttt{dcenn.yaml}).}
This file defines the core encoder hyperparameters: latent dimension (\texttt{enc\_dim}), recurrent steps (\texttt{steps}), block size (\texttt{block}), autoencoder epochs (\texttt{ae\_epochs}), learning rate, weight decay, optional subsampling, and random seed \cite{repo_thesis_grid_anomaly}. These parameters govern model capacity, training cost, and latent-structure regularization.

\paragraph{ELM configuration (\texttt{elm.yaml}).}
This file contains the ridge regularization parameter \texttt{l2} for the analytical forecasting head \cite{repo_thesis_grid_anomaly}. Although minimal, it is conceptually important because it controls the bias--variance trade-off in the closed-form output layer.

\paragraph{Threshold configuration (\texttt{thresholds.yaml}).}
This file governs the anomaly calibration stage through the thresholding method, confidence level, and bucketing scheme, e.g., \texttt{method = conformal}, \texttt{alpha = 0.90}, and \texttt{bucket\_by = hour} \cite{repo_thesis_grid_anomaly}. These parameters determine how residual uncertainty is translated into anomaly decisions.
